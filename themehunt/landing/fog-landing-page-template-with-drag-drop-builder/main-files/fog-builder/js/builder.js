...
var editableContent = ['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'ul', '.editContent', '.navbar a', 'button', 'a.btn', '.footer a:not(.fa)', '.tableWrapper'];
...
$(function(){
	...
	|----------------------------------------------------------------------------
	*/
	//color bar switch on modal
	$('.color-bar li').on('click', function(event) {
		event.preventDefault();
		/* Act on the event */
		$('.color-bar .active').removeClass('active');
		$(this).addClass('active');
	});

	//define global variable
	var colorCSSRef = $('.color-bar .active').attr('class').split(' ')[0],
		 // typographyCSSRef = 'lane',
		 frameStyleToChange = document.getElementById('pageList').getElementsByTagName('iframe');

	//stylesheet update into final file
	var styleChangeTo = function(to, dir, ref) {
		document.getElementById("skeleton").contentWindow.document.getElementById( to ).setAttribute( 'href', 'css/' + dir.toLowerCase() + '/' + ref + '.css' );
	}

	//stylesheet update into builder
	$('#styleSwitch').off('click', '#styleSwitchConfirm').on('click', '#styleSwitchConfirm', function(){

		$('#styleSwitch').modal('hide');

		//assign value to the variable
		colorCSSRef = $('.color-bar .active').attr('class').split(' ')[0];
		// typographyCSSRef = $('select', '#form-typography').val().toLowerCase();

		//preview block CSS
		$(frameStyleToChange).each(function() {

			this.contentWindow.document.getElementById('fromColor').setAttribute('href', 'css/colors/' + colorCSSRef + '.css');

			// this.contentWindow.document.getElementById('fromTypography').setAttribute('href', 'css/typography/' + typographyCSSRef + '.css');
		});
	});
	...
	$('#previewModal').on('shown.bs.modal', function (e) {
		...
		$('#pageList > ul:visible').each(function(){
			...
			$(this).find('iframe').each(function(){
				...
				styleChangeTo('toColor', 'colors', colorCSSRef);
				// styleChangeTo('toTypography', 'typography', typographyCSSRef);
